    ...
    async def _lookup_reply_target(self, reply_message_id: str) -> Optional[Tuple[str, str, str, float]]:
        ...
        if cached is not None:
            ...
            if now - cached[0] <= ttl:
                if not cached[1]:
                    self.ctx.logger.debug(
                        "回复目标查询：命中**负缓存**（msg_id=%s，%.0f 秒内不再重试）", reply_message_id, ttl
                    )
                    return None
                self.ctx.logger.debug(
                    "回复目标查询：命中缓存（msg_id=%s → user_id=%s 昵称=%s 群名片=%s）",
                    reply_message_id,
                    cached[1],
                    cached[2] or "-",
                    cached[3] or "-",
        ...
        if target is None:
            ...
            self.ctx.logger.info(
                "回复目标查询失败（msg_id=%s）：查不到被回复消息的发送者，本次不 @ 目标"
                "（抽到 at/quote_at 时会回退为引用回复）",
                reply_message_id,
            )
            return None
        ...
        self.ctx.logger.debug(
            "回复目标查询成功（msg_id=%s → user_id=%s 昵称=%s 群名片=%s 时间戳=%.0f）",
            reply_message_id,
            user_id,
            nickname or "-",
            cardname or "-",
