...
「@某人 + 后面的空白」整段删掉，这样「引用回复接管」抽到 @ 时注入的**真实 @** 才是消息里唯一的一个。
两个限定条件同时满足才动手：① 在消息正文最前面；② `@名字` 后面跟着空白
（半角/全角空格、Tab、NBSP，**0.14.3 起换行也算**——LLM 爱把 `@某人` 单独放一行）。
所以 `a@b.com`、`@某人，` 这类写法不会被误伤；删完只剩空白时也不删（免得发出空消息）。
作用范围与文本规则一致（只动 bot 本轮回复自己发出的消息，含插件补发的分段），
并**与两个接管同口径受「丰富回复门控」约束**。

> **0.14.3 起按"逻辑串"匹配组件，堵住两个残留空格的漏网点**（排查报告 L1/L2）：
> 宿主/别的插件会把正文拆成多个组件——① `@` 段**前面**若有纯空白文本段，旧实现原样
> 留着，渲染时 join 再补一个空格，目视就是前导双空格，现在会一并清掉（与注入真实 @
> 前的 `_strip_body_leading_whitespace` 同口径）；② `@名字` 与它后面的空白**分属两个
> 组件**时旧正则只见单段、永远匹配不上，现在把相邻文本段拼成一个逻辑串再匹配。
> 另外尾随空白从"只认水平空白"扩成**含换行**：`@某人\n正文` 的假 @ 现在也会被过滤
> （0.14.3 前那种写法永远漏网，之后接管一注入真实 at 就又是两个 @）。

> **0.14.2 起只删「本轮回复目标」那个名字。** 这条过滤的初衷是清掉 LLM 照抄上下文里的
> `@昵称`（那必然就是被回复的人），原实现却无条件删。于是当 LLM 主动写 `@张三` 而本轮
> 回复的是李四时：`@张三` 被删掉，随后抽到 @ 又会注入**李四**的真实 at —— 等于把话 @
> 给了另一个人（同时让"@ 的是别人就跳过注入"那条分支成了死代码）。现在只在字面昵称与
> 回复目标的**昵称/群名片**任一相等时才删；@ 的是别人就原样保留，交给引用回复接管按原口径
> 处理（抽到 @ 时跳过注入、只统一空格）。目标查不到时（不是回复 / 消息被清理）仍按原行为
> 删除——那种情况下注入路径同样拿不到目标、会退化为引用回复，不会 @ 错人。

**插件不写任何配置文件**（0.13.18 起）：`config.toml` 由**宿主**生成与维护
（首次生成、配置版本变化、WebUI 保存），插件只在加载时**读**它。
每个配置项的说明文案在三处同源：**`config.toml` 的行尾注释**（宿主保留）、
**WebUI 的字段提示**（插件通过 `get_webui_config_schema` 补 `hint`）、**本文档的字段表**。
...
| `filter_fake_at` | `True` | 删掉正文开头的「@某人 + 后面的空白」（空格/Tab/换行，0.14.3 起跨组件也算）——**只删本轮回复目标的名字**（0.14.2 起），@ 的是别人则保留。 |
...
| `config_version` | 当前插件版本 | 配置版本，与插件版本同步（自动维护，不要手改）。 |
...
>
> **0.13.10 ~ 0.14.0 期间这个开关不生效**（0.14.1 修）：字段住在 `[plugin]`，代码却去
> `[quote_reply]` / `[quote_reply_private]` 里读，`getattr` 取不到便一路兜底成 `False`
> —— 那段时间在 1.2.3 上无论怎么打开，@ 与正文始终粘连。升级到 **0.14.1** 后按上表设置即可。
...
> [`[plugin] filter_fake_at`](#plugin)——**0.14.2 起只删本轮回复目标的名字**；@ 的是别人时
> 无论开关开不开都按 @ 判重处理（命中本轮目标就升级成真实 at 组件、是别人则跳过注入），
> 所以一条消息里既不会出现两个 @，也不会把话 @ 给另一个人。
...
- **时序守卫**：补发期间 planner 请求与新入站消息会先等待补发完成（超时放行）。
  ...
  注册的 `timeout_ms` 是 **58000（58 秒）**，**刻意不高于宿主全局默认的 60 秒阻塞超时**
  （0.14.3 起从 65 秒下调）：内部等待上限是 `wait_timeout_seconds`（默认 30 秒），
  钩子上限设得更宽才能让"等补发"由配置而不是由 Hook 超时来决定（Hook 超时会让宿主
  直接放行并记一条告警）。**配套钳制**：内部等待被钳到最长 55 秒（钩子上限 − 3 秒
  收尾余量）——就算把 `wait_timeout_seconds` 调得再大，"超时放行"也永远由插件自己的
  等待逻辑干净地结束（带日志），而不是被宿主在 Hook 超时处掐断。
  **代价要讲清楚**：极端情况下（补发卡住且 `wait_timeout_seconds` 调到 55 以上）这个
  blocking 处理器最多会让**新入站消息**的处理等 55 秒。不想承担就把
  `wait_timeout_seconds` 调小或设 `0`（完全不等待）；模型处理不过来、补发经常等不完时
  再手动调大它（55 秒以内都全额生效）。
...
`quiet_seconds` 秒**且本轮工具链已经停下来**、且本轮没有出现表情包组件时，按
`probability` 概率抽取一张：

- `emotion` 留空 → 随机抽取；
- `emotion` 填情绪标签（即表情包描述/情绪词，如 `开心`）→ 按情绪抽取，取不到就跳过。

**为什么"静默"还不够（0.14.2）**：planner 是 **action loop** —— `reply` 把消息发出去之后，
宿主会带着工具结果**再请求一次模型**，模型完全可能在**下一轮**才调 `send_emoji`
（该工具内部还要跑视觉子代理选图，数秒到数十秒）。只按"最后一条出站消息静默 N 秒"
判定，4 秒的窗口很容易落在"下一轮请求还没回来"或"正在选图"的中间，于是插件先补了一张、
宿主的 `send_emoji` 随后又发一张，群里出现**两个表情包**。现在判定条件是**两个都满足**：

1. 出站静默 ≥ `quiet_seconds`；
2. **主 planner 没有在途请求** —— `maisaka.planner.before_request` 里
   `tool_definitions` 非空即视为"主 planner 有一轮在跑"，其 `after_response` 回来才清掉
   （子代理请求的 `tool_definitions` 为空，不参与判定）。

等待上界是本轮有效期减一个轮询间隔，到点就按"不再等"放行，保证补发仍然来得及判定
（等满 `round_window_seconds` 反而会被当成"太晚"直接丢弃）。

**planner 自己打算发表情时不会补发（0.7.0 + 0.14.2 双保险）**：

- **第一道（0.7.0，按工具名）**：`maisaka.planner.after_response` 里读本轮 `output_items`
  的工具调用名，命中 `planner_emoji_tools` 即打标记（有效期 `planned_emoji_ttl_seconds`），
  本轮跳过补发；planner 的表情真的发出后标记立即清除。**默认只含 `send_emoji`**；
  想连"贴表情"一起算就把 `emoji_like` / `emoji_like_list` 自己加进去（贴表情同样表达情绪）；
  留空则关闭该判定。
- **第二道（0.14.2，按事实）**：宿主 `send_emoji` 内置工具在真正选图前会触发
  `emoji.maisaka.before_select`（`stream_id` 就是会话 ID），插件收到就把本轮标记成
  "已有表情包在路上"。这条**不依赖上面那个工具名名单**，因此名单配漏/配错时也不会多发。

> 两道都依赖宿主输出项快照里 `FunctionCallItem.tool_call.func_name` 的形态（形态在
> MaiBot 1.2.5 源码里核对，插件本体在 1.2.3 上实跑）：形态不匹配时第一道静默退化为
> "只看已发出的出站消息"，第二道不受影响。本插件自己发图走 `ctx.send.emoji` 能力、
> 不经过宿主那个内置工具，所以不会自己把自己标记掉。
...
`round_window_seconds` 限制一轮记录的最长存活时间，同时是"等 planner 收尾"的上界。
planner 先发表情后回复（15 秒内）时同样不会补发。发送前统一受聊天流表情冷却约束（见下节）。
...
| `quiet_seconds` | `4.0` | 群聊安静这么久、**且 planner 本轮不再有在途请求**时才考虑补发。 **取值范围** `0.5~`。 |
| `round_window_seconds` | `90.0` | 判定「同一轮回复」的时间窗，也是等待 planner 收尾的上界。 **取值范围** `1~`。 |
...
`emoji.register.after_build_description`）会把描述**刷新**进库里。

> **0.13.18 修了一个由此引发的"永久无法注入"**：库里的记录只存**一条**描述，而宿主那份描述
> 可能与它不一致（最常见：含义是在"宿主还没出描述"时按 hash 算出来的，库里 `description=''`；
> 也可能是宿主重新生成过描述）。此时**注入侧**按 hash 需要本地 `msg_id → refs` 缓存
> （TTL 有限、插件重启即清空），拿不到就只能按描述匹配，而描述又对不上 → **双双未命中**；
> **补录扫描**此前只判 `hash in known_hashes()`（**只看 hash**）→ 认为"已有含义"直接跳过
> → 这些表情包**永远不会被重新收录**（服务器实测：45 次注入未命中 vs 8 条新收录）。
>
> 现在：① 库里新增 `emoji_meaning_aliases`（描述 → hash）表，**旧描述不丢**；
> ② 新增 `refresh_description()`：描述不一致时**就地更新**（旧的非空描述转存为别名），
> **不重新调 VLM**（含义讲的是图，与描述无关）；③ 扫描与两个回填入口都改用它；
> ④ 按描述回退匹配时**别名也认**，新旧标签都能解析。
...
- **描述刷新有三个入口，入站载荷那个最早**（0.13.5 起，0.13.18 起改为"空与非空都能修"）：
  ① 入站钩子载荷里组件的 `data` 就是宿主渲染后的 `[表情包: 描述]`；
  ② `emoji.register.after_build_description` 钩子；③ **planner 按需补录扫描**（0.13.18 新增）。
  三处都调 `refresh_description()`：描述与库里不一致就更新，旧的非空描述转存为**别名**，
  所以描述变了也不会让旧标签失效。① 比 ② 可靠——插件晚加载 / 重启错过的描述，
  下次该表情出现时也能补上；③ 覆盖"消息在 bot 自己那边、入站钩子看不到"的情况。
- **本地映射是纯缓存**：`message_id → [(hash, 描述)]` 容量 512 条、TTL 1 小时、LRU 淘汰，
  **插件重启即清空**。所以"按 hash 注入"对**较老的消息**往往不可用，只能退到按描述匹配 ——
  这也是 0.13.18 那个"描述不一致"问题会长期潜伏的原因。
- **本地映射是纯缓存，不需要落库**：`message_id → [(hash, 描述)]` 在入站那一刻零成本记下，
  丢了由 `get_by_id` 兜底（宿主消息库里本来就有 `message_id` + 组件 `hash`）。
  容量 512 条、TTL 1 小时，LRU 淘汰。

**替换范围（0.13.7）**：`rewrite_scope`（默认 `["replyer", "planner"]`，与下方字段表、代码默认值一致）决定改写哪些请求的上下文：
...
> **扫描日志（0.13.18 起分类计数）**：`planner 按需补录：本轮上下文 N 个表情包 → 新入队 X，
> 描述已刷新 Y（已有含义 A / 已在队列 B / 已放弃 C / 无 hash D，会话 …）`。
> 此前只在"有新入队"时才打 INFO，且不区分"已有含义 / 已在队列 / 已放弃"——
> **正是这句含糊的日志让"永久无法注入"长期查不出来**（它把"描述对不上"也报成"已有含义"）。
...
- **bot 自己发出的表情包现在也能被识别与注入**（0.14.0 修）。修之前有两层问题：

  1. **不进上下文**：宿主 `send.emoji` 能力的 `sync_to_maisaka_history` **默认 `False`**，
     插件没传 → 这条表情消息只入库、**不进 bot 的对话上下文** → bot 自己发过的表情
     在上下文里完全不存在。现在显式传 `sync_to_maisaka_history=True`
     （参数对齐宿主自己的 `send_emoji` 工具：`storage_message=True` / `set_reply=False` /
     `reply_message=None` / `maisaka_source_kind="guided_reply"`）。
  2. **即使进了上下文也没有标签**：宿主渲染**发送侧**的 emoji 组件用的是 `component.content`
     （`maisaka/context/message_adapter.py`），而发送时构造的组件只带 `binary_hash`、
     **不带描述** → 上下文里只有**无标签的 `[表情包]`**。所以"按描述注入"走不通，
     **只能靠"按 hash"**。为此插件补了两件事：
     - **出站观察者**（`send_service.after_send`）按**内部 `message_id`** 记一份
       `msg_id → [(hash, 描述)]` 映射 —— 上下文里的 `[msg_id:…]` 正好能与它对上，
       "按 hash 注入"因此成立（出站侧以前从不记录）；
     - **发送前登记含义库**：hash 本地算（`sha256_of_base64`，与宿主
       `_build_binary_component_from_base64` 同口径），描述用**宿主给的那份**
       （`emoji.get_by_description` / `get_random` 的载荷里本来就有 `description`，以前被丢掉了）：
       库里已有该 hash 时**刷新描述**（旧描述转别名），没有则**带描述入队补录** ——
       注入文本里的标签才是对的。
...
| 2 | 表情包图片：`data/images/<hash>.<ext>` 或 `Images.full_path` | 含义库生成时取图（`_load_emoji_bytes_by_hash`） | **根目录包含校验**（拒绝宿主根之外的路径）+ **单文件 8MB 上限** |
