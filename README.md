...
| 表情包跟风 | 开 | 群友**连续**发送 `threshold`（默认 `2`）条表情包后，按配置方式跟发一张（与最后一个同情绪 / 指定情绪 / 纯随机）；**中间插入任何非表情包内容（含系统通知、bot 自己的发言）都会重新计数**；不计入 bot 自身的表情包与插件注入的合成记录 |
...
通过 `chat.receive.after_process` 统计群聊中**连续**入站的表情包条数，达到 `threshold`
（**默认 2**）后按 `probability` 概率跟发一张：
...
**中间插入任何一个非表情包信息，连击立刻清零**，下一张表情包从 1 重新数起。会打断的情况：

| 插入的内容 | 是否打断 | 说明 |
|---|---|---|
| 群友的文本 / 图片 / 语音 / 其它非表情包消息 | **打断** | |
| **系统通知**（戳一戳、撤回、入群提示等 `is_notify` 记录） | **打断** | 0.11.1 及更早是"不计数也不打断"，等于被跳过；0.11.3 起按"插入的非表情包信息"处理 |
| **bot 自己发出的文本** | **打断** | 0.11.1 及更早被 self 过滤顺带跳过；0.11.3 起视为插入的内容 |
| **其它插件注入的合成记录**（`*injected*`，且不含表情包） | **打断** | 同上 |
| **bot 自己 / 注入的「表情包」回环** | 不打断 | 它**本身就是表情包**，不是"非表情包信息"；而且如果让它打断，本插件跟发的那一张会把自己的连击清掉。它也不计数（见下） |
| 无法归属会话的载荷（没有 `session_id`） | 不打断 | 既不能计数也不知道该清谁的连击 |

**含表情包组件的混合消息**（如「哈哈哈 + 表情包」）按"发了表情包"处理，**继续**连击而不是打断：
用户确实发了表情包，而且这不属于"两条消息之间插入了别的东西"。
...
`self_id`/`platform_io_account_id` 与发送者比对，剔除 bot 自身的**表情包**与 `*injected*`
的**表情包**（不计入、也不打断、也不进含义缓存）；关掉本项后它们按普通群消息处理（会计数）。
...
> **默认值变更（0.11.3）**：`threshold` 由 `3` 改为 `2`。老配置里若还写着 `3`，升级时会
> 被当作"没改过"丢弃、按新默认 `2` 生效；你要是当时就想要 `3`，手改回 `3` 即可。
...
| `threshold` | `2` | 群友连续发多少条表情包后触发跟发（默认 2）。中间插入任何非表情包内容（含系统通知与 bot 自己的发言）都会重新计数。 **取值范围** `1~`。 |
